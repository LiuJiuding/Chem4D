...
class PDTTranslation():
    ...
    def listtoc4d(mylist:list):
        if len(mylist) == 3 and type(mylist[0]) != list:
            return c4d.Vector(mylist[0],mylist[1],mylist[2])
        else:
            v = []
            for i in mylist:
                v.append(c4d.Vector(i[0],i[1],i[2]))
            return v
...
class C4DFunction(object):
    ...
    @staticmethod
    def CreateMesh(geo:PDTGeo) ->c4d.BaseObject:
        mesh = c4d.BaseObject(5100)
        mesh.ResizeObject(len(geo.points))
        mesh.SetAllPoints(geo.GetP())
    ...
    def CreateAtom(geo:PDTGeo):
        ...
        labels = geo.GetPointAttrib("label")
    ...
    @staticmethod
    def CreateBond(geo_bond:PDTGeo, attrname:str) ->c4d.BaseObject:
